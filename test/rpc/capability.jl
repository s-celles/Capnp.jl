...
@testset "Remote capability equality" begin
    # Cap'n Proto guarantees reference equality for capabilities received from a
    # single peer. Here that means: same import ID on the same connection
    # denotes the same object.
    conn = RPC.Connection(RPC.MockTransport())
    other = RPC.Connection(RPC.MockTransport())

    @testset "Repeated import yields one object" begin
        first = RPC.import_capability!(conn, RPC.ImportId(7))
        again = RPC.import_capability!(conn, RPC.ImportId(7))

        @test again === first
        @test again == first
        @test isequal(again, first)
        @test RPC.import_count(conn) == 1
    end

    @testset "Each sighting takes a reference" begin
        cap = RPC.import_capability!(conn, RPC.ImportId(9))
        @test cap.ref_count == 1
        RPC.import_capability!(conn, RPC.ImportId(9))
        RPC.import_capability!(conn, RPC.ImportId(9))
        @test cap.ref_count == 3
    end

    @testset "Distinct imports are not equal" begin
        a = RPC.import_capability!(conn, RPC.ImportId(1))
        b = RPC.import_capability!(conn, RPC.ImportId(2))
        @test a != b
        @test !isequal(a, b)
    end

    @testset "Equality does not cross connections" begin
        # The same import ID on a different peer denotes a different object.
        # Deciding otherwise would require Join, which belongs to a network
        # type this package does not target.
        here = RPC.import_capability!(conn, RPC.ImportId(42))
        there = RPC.import_capability!(other, RPC.ImportId(42))

        @test here !== there
        @test here != there
        @test hash(here) != hash(there)
    end

    @testset "Reference count is excluded from identity" begin
        cap = RPC.import_capability!(conn, RPC.ImportId(100))
        before = hash(cap)
        RPC.incref!(cap)
        @test cap.ref_count == 2
        @test hash(cap) == before
        @test cap == RPC.get_import(conn, RPC.ImportId(100))
    end

    @testset "Usable as a dictionary key" begin
        cap = RPC.import_capability!(conn, RPC.ImportId(55))
        seen = Dict(cap => :recorded)
        @test seen[RPC.import_capability!(conn, RPC.ImportId(55))] == :recorded
        @test !haskey(seen, RPC.import_capability!(other, RPC.ImportId(55)))
    end

    @testset "Import table limit still applies" begin
        small = RPC.Connection(RPC.MockTransport(), max_imports = 2)
        RPC.import_capability!(small, RPC.ImportId(1))
        RPC.import_capability!(small, RPC.ImportId(2))
        # A repeated import must not count against the limit.
        @test RPC.import_capability!(small, RPC.ImportId(1)) !== nothing
        @test_throws Exception RPC.import_capability!(small, RPC.ImportId(3))
    end
end
