...
# Helper capabilities for the general results-struct test at the end of this file.
# Defined at top level because a `module` and `dispatch_method!` methods cannot
# live inside a `@testset` block.
module GeneralResults
using Capnp
using Capnp.RPC

const INTERFACE_ID = UInt64(0x1234_5678_9abc_def0)
...
function write_text!(ptr, pointer_index, text)
    loc = Capnp.WirePointer(ptr.segment, ptr.offset + ptr.data_word_count + pointer_index)
    loc, seg, off = Capnp.alloc(ptr.traverser, loc, ncodeunits(text) + 1)
    child = Capnp.SimpleListPointer{UInt8,typeof(ptr.traverser)}(ptr.traverser, seg, off, Capnp.Byte, UInt32(ncodeunits(text) + 1))
    Capnp.write_list_pointer(loc, child)
    Capnp.write_text(child, text)
end
read_text(ptr, pointer_index) = Capnp.read_text(Capnp.read_list_pointer(ptr, ptr.data_word_count, pointer_index))
results_of(payload) = Capnp.read_struct_pointer(payload, 0, 0)

# lookup() -> (count :UInt64, label :Text, greeter :Greeter)
function Capnp.RPC.dispatch_method!(::Registry, ::UInt64, ::UInt16, ctx::RPC.CallContext, params)
    RPC.set_results!(ctx; data_word_count = 1, pointer_count = 2) do results, _
        Capnp.write_bits(results, 0, UInt64, UInt64(7))
        write_text!(results, 0, "directory")
        RPC.write_capability!(ctx, results, 1, Greeter("hello"), INTERFACE_ID)
    end
end

# greet() -> (text :Text)
Capnp.RPC.dispatch_method!(impl::Greeter, ::UInt64, ::UInt16, ctx::RPC.CallContext, params) = RPC.set_result!(ctx, impl.greeting * ", world")
end # module GeneralResults
...
@testset "Results struct with data, text and a capability" begin
    # Exercised over a real connection: a capability carried in a pointer slot
    # must be callable directly and reachable by pipelining on that same slot.
    server = RPC.Server(GeneralResults.Registry())
    RPC.listen(server, "127.0.0.1", 0)
    port = Int(Sockets.getsockname(server.tcp_server)[2])
    server_task = @async RPC.serve(server)

    conn = nothing
    try
        conn = RPC.connect("127.0.0.1", port)
        registry = RPC.bootstrap(conn, RPC.RemoteCapability)

        payload = fetch(RPC.call(registry, GeneralResults.INTERFACE_ID, UInt16(0)))
        results = GeneralResults.results_of(payload)
        @test Capnp.read_bits(results, 0, UInt64) == 7
        @test GeneralResults.read_text(results, 0) == "directory"

        # The capability in pointer slot 1 reached the client
        @test length(payload.traverser.capabilities) == 1
        greeter = payload.traverser.capabilities[1]
        @test greeter isa RPC.RemoteCapability

        # ... and is callable, which needs MessageTarget.importedCap on the wire
        greeting = fetch(RPC.call(greeter, GeneralResults.INTERFACE_ID, UInt16(0)))
        @test GeneralResults.read_text(GeneralResults.results_of(greeting), 0) == "hello, world"

        # ... and reachable by pipelining on pointer slot 1, which needs the
        # PromisedAnswer transform on the wire and slot-aware resolution.
        promise = RPC.call(registry, GeneralResults.INTERFACE_ID, UInt16(0))
        pipelined = RPC.call_pipelined(promise, [RPC.PipelineOp(RPC.PipelineOpKind.GET_POINTER_FIELD, UInt16(1))])
        piped = fetch(RPC.call(pipelined, GeneralResults.INTERFACE_ID, UInt16(0)))
        @test GeneralResults.read_text(GeneralResults.results_of(piped), 0) == "hello, world"

        # A target the server cannot resolve is refused, not quietly answered by
        # some other capability. Export id 0 is reserved, so it names nothing.
        for bogus_id in (RPC.ImportId(0), RPC.ImportId(999))
            bogus = RPC.import_capability!(conn, bogus_id)
            @test_throws RPC.RemoteException fetch(RPC.call(bogus, GeneralResults.INTERFACE_ID, UInt16(0)))
        end

        # Pipelining on a question the server no longer holds is refused too
        stale = RPC.Promise{Any}(question_id = RPC.QuestionId(4242), connection = conn, pipeline_ops = [RPC.PipelineOp(RPC.PipelineOpKind.GET_POINTER_FIELD, UInt16(0))])
        @test_throws RPC.RemoteException fetch(RPC.call(stale, GeneralResults.INTERFACE_ID, UInt16(0)))
    finally
