...
Two remote capabilities are equal when they denote the same object on the same
peer, that is when they carry the same import ID on the same connection. This
is the reference-equality guarantee Cap'n Proto describes for capabilities
received from a single vat; comparing capabilities across vats would require
`Join`, which belongs to a network type this package does not target.

The reference count is deliberately excluded from equality: it tracks how many
times the local side holds the capability, not which object is denoted.
"""
...
# Identity is (connection, import ID). `import_capability!` already returns the
# same object for a repeated import, so these usually compare identical values;
# defining them explicitly means the guarantee holds even for a capability
# rebuilt outside that path, rather than resting on every call site remembering
# to look the table up first.
Base.:(==)(a::RemoteCapability, b::RemoteCapability) = a.import_id == b.import_id && a.connection === b.connection
Base.isequal(a::RemoteCapability, b::RemoteCapability) = a == b
Base.hash(cap::RemoteCapability, h::UInt) = hash(cap.import_id, hash(objectid(cap.connection), h))
...
mutable struct PendingAnswer
    ...
    result::Any
    # Which pointer slot of the results struct holds which capability. Empty when
    # the answer was built by a path that does not track slots, in which case
    # pipelined lookups fall back to the position in `result_caps`.
    cap_slots::Dict{UInt16,ExportId}

    PendingAnswer(aid::AnswerId, caps::Vector{ExportId} = ExportId[], refs::UInt32 = UInt32(0); cap_slots::Dict{UInt16,ExportId} = Dict{UInt16,ExportId}()) = new(aid, caps, refs, nothing, cap_slots)
end
...
mutable struct Connection
    ...
    function Connection(transport::Transport; owns_transport::Bool = true, inbound_queue_size::Int = 64, outbound_queue_size::Int = 64, max_questions::Int = 1024, max_answers::Int = 1024, max_exports::Int = 8192, max_imports::Int = 8192)
        ...
        new(
            ...
            0,
            # Export ids start at 1 so that a zeroed or missing id on the wire
            # never names a real capability.
            1,
            nothing,
            ReentrantLock(),
            owns_transport,
...
"""
    import_capability!(conn::Connection, iid::ImportId; interface_id = UInt64(0)) -> RemoteCapability

Return the capability for `iid` on `conn`, creating and registering it on first
sight and taking a reference on every later sight.

This is the only supported way to turn an import ID into a `RemoteCapability`.
Going through it is what makes two references to one import compare equal, and
it holds the connection lock across the lookup and the insert, so two receive
paths racing on the same import cannot each register a separate object.
"""
function import_capability!(conn::Connection, iid::ImportId; interface_id::UInt64 = UInt64(0))
    lock(conn.lock) do
        existing = get(conn.imports, iid, nothing)
        if existing !== nothing
            existing.ref_count += 1
            return existing
        end
        _check_table_limit(conn.imports, iid, conn.max_imports, :imports)
        cap = RemoteCapability(iid, interface_id, conn)
        conn.imports[iid] = cap
        return cap
    end
end
...
export add_import!, get_import, import_capability!, remove_import!
