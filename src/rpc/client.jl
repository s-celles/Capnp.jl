...
`tcp_nodelay` controls the `TCP_NODELAY` socket option on TCP connections,
including TLS connections made through the Reseau extension. It defaults to
`true`, matching the reference Cap'n Proto implementation; set it to `false` to
keep Nagle's algorithm enabled. It is ignored by transports that have no such
option, such as Unix-domain sockets.
"""
struct ConnectionOptions
    ...
    function ConnectionOptions(;
        ...
        new(max_message_size, max_segments, traversal_limit_words, nesting_limit, inbound_queue_size, outbound_queue_size, max_questions, max_answers, max_exports, max_imports, tcp_nodelay)
    end
end
...
function connect(host::AbstractString, port::Integer, options::ConnectionOptions)
    transport =
        TcpTransport(host, port; max_message_size = options.max_message_size, max_segments = options.max_segments, traversal_limit_words = options.traversal_limit_words, nesting_limit = options.nesting_limit, nodelay = options.tcp_nodelay)
                    ...
                    if return_msg.kind == ReturnType.RESULTS
                        ...
                        for cap_desc in return_msg.cap_table
                            if cap_desc.kind == CapDescriptorType.SENDER_HOSTED
                                ...
                                cap = import_capability!(conn, import_id)
    ...
    if resolve.kind == ResolveType.CAP
        ...
        elseif cap_desc.kind == CapDescriptorType.SENDER_HOSTED
            ...
            if new_import_id !== nothing
                ...
                remote_cap = import_capability!(conn, new_import_id)
    ...
    on_resolve!(promise, function (value)
        if value isa ParsedRestoreResults
            if value.success && value.import_id !== nothing
                ...
                remote_cap = import_capability!(conn, value.import_id)
