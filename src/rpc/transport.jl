...
Default value of the `nodelay` transport option. Cap'n Proto's reference C++
implementation sets `TCP_NODELAY` on every RPC socket: the protocol exchanges
many small messages whose latency matters far more than the bandwidth Nagle's
algorithm saves, and pipelined calls otherwise stall waiting for an ACK.
"""
const DEFAULT_TCP_NODELAY = true

"""
    set_tcp_nodelay!(socket, enable::Bool = true) -> Bool

Disable (`enable = true`) or re-enable Nagle's algorithm on a TCP socket or
listener, i.e. set the `TCP_NODELAY` socket option. Returns `true` when the
option was applied.

Objects with no such option (Unix-domain sockets, TLS streams, in-memory
transports) and sockets that are already closed are left alone and return
`false`, so callers can apply this unconditionally.
"""
function set_tcp_nodelay!(socket, enable::Bool = true)
    socket isa Sockets.TCPSocket || socket isa Sockets.TCPServer || return false
    isopen(socket) || return false
    Sockets.nagle(socket, !enable)
    return true
end
...
`TCP_NODELAY` is set on the socket by default (`nodelay = true`), matching the
reference Cap'n Proto implementation. Pass `nodelay = false` to keep Nagle's
algorithm enabled when throughput matters more than per-message latency.
"""
mutable struct TcpTransport <: Transport
    ...
    function TcpTransport(
        ...
        traversal_limit_words::Int = Capnp.DEFAULT_TRAVERSAL_LIMIT_WORDS,
        nesting_limit::Int = Capnp.DEFAULT_NESTING_LIMIT,
        nodelay::Bool = DEFAULT_TCP_NODELAY,
    )
        Capnp._validate_reader_limits(max_message_size, max_segments)
        Capnp._validate_traversal_limits(traversal_limit_words, nesting_limit)
        set_tcp_nodelay!(socket, nodelay)
...
function TcpTransport(
    ...
    TcpTransport(Sockets.connect(host, port); max_message_size, max_segments, traversal_limit_words, nesting_limit, nodelay)
end
...
export set_tcp_nodelay!, DEFAULT_TCP_NODELAY
